...
### 在JavaScript 中创建元素，用createElement()还是innerHTML好一点？

- 最叼的好像是 使用[模板技术](https://github.com/xwcoder/template),其实我的静态博客用的就是这种。

- 在用js代码的循环创建元素的时候就顺便一个个绑定好事件好像比较简单，这样就不用给每个元素增加属性了。

  那么，用createElement来创建元素和用innerHTML，不知道哪个比较好用呢??

  而如果用innerHTML来创建元素的话肯定不能在形成元素的循环里面给元素绑定事件了，这样太硬编码了。
...
- > 《高性能JavaScript》：
  ...
  > - 如果要使用createElement()，推荐一下createDocumentFragment()
